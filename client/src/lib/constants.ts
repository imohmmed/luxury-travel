export const slides = [
  {
    id: 1,
    image: 'https://images.unsplash.com/photo-1566073771259-6a8506099945?ixlib=rb-4.0.3&auto=format&fit=crop&w=1920&h=1080',
    title: 'التَرَف للسفر والسياحة',
    subtitle: 'تجارب سفر استثنائية تفوق توقعاتك',
    buttonText: 'احجز رحلتك الآن',
    buttonLink: '#services',
    buttonVariant: 'accent',
  },
  {
    id: 2,
    image: 'https://images.unsplash.com/photo-1542314831-068cd1dbfeeb?ixlib=rb-4.0.3&auto=format&fit=crop&w=1920&h=1080',
    title: 'خدمات التَرَف المتكاملة',
    subtitle: 'من الفيز إلى التأمين الصحي وإجازات السوق الدولية',
    buttonText: 'استكشف خدماتنا',
    buttonLink: '#services',
    buttonVariant: 'primary',
  },
  {
    id: 3,
    image: 'https://images.unsplash.com/photo-1540339832862-474599807836?ixlib=rb-4.0.3&auto=format&fit=crop&w=1920&h=1080',
    title: 'خدمات ترانسفير VIP',
    subtitle: 'استقبال وتوديع بأعلى مستويات الفخامة والراحة',
    buttonText: 'احجز خدماتنا',
    buttonLink: '#services',
    buttonVariant: 'white',
  },
];

export const services = [
  {
    id: 1,
    image: 'https://images.unsplash.com/photo-1591261730899-5a78d7cddcee?ixlib=rb-4.0.3&auto=format&fit=crop&w=600&h=400',
    title: 'قسم الفيز',
    description: 'نقدم خدمات متكاملة لاستخراج التأشيرات السياحية والتجارية لكافة دول العالم بإجراءات سريعة وموثوقة'
  },
  {
    id: 2,
    image: 'https://images.unsplash.com/photo-1532635224-cf024e9d1904?ixlib=rb-4.0.3&auto=format&fit=crop&w=600&h=400',
    title: 'قسم الكروبات السياحية وتنظيم المؤتمرات',
    description: 'خدمات احترافية لتنظيم رحلات المجموعات والمؤتمرات العالمية بأعلى مستويات الجودة والاهتمام بالتفاصيل'
  },
  {
    id: 3,
    image: 'https://images.unsplash.com/photo-1540962351504-03099e0a754b?ixlib=rb-4.0.3&auto=format&fit=crop&w=600&h=400',
    title: 'قسم التذاكر',
    description: 'احجز تذاكر الطيران على الدرجة الأولى ودرجة رجال الأعمال مع أفضل شركات الطيران العالمية بأسعار تنافسية'
  },
  {
    id: 4,
    image: 'https://images.unsplash.com/photo-1600880292203-757bb62b4baf?ixlib=rb-4.0.3&auto=format&fit=crop&w=600&h=400',
    title: 'قسم إجازات السوق الدولية',
    description: 'خدمات استخراج وتجديد رخص القيادة الدولية لتمكينك من قيادة السيارات أثناء سفرك في مختلف دول العالم'
  },
  {
    id: 5,
    image: 'https://images.unsplash.com/photo-1504813184591-01572f98c85f?ixlib=rb-4.0.3&auto=format&fit=crop&w=600&h=400',
    title: 'قسم التأمين الصحي',
    description: 'نوفر خدمات التأمين الصحي للمسافرين بتغطية شاملة في جميع أنحاء العالم لضمان رحلة آمنة ومطمئنة'
  },
  {
    id: 6,
    image: 'https://images.unsplash.com/photo-1523475743164-9191dafcb0d6?ixlib=rb-4.0.3&auto=format&fit=crop&w=600&h=400',
    title: 'قسم الاستقبال والتوديع (ترانسفير)',
    description: 'خدمات VIP للاستقبال والتوديع في المطارات العالمية مع سيارات فاخرة وسائقين محترفين لراحة تامة'
  }
];

export const destinations = [
  { id: 1, name: 'جزر المالديف', icon: 'fas fa-umbrella-beach' },
  { id: 2, name: 'باريس', icon: 'fas fa-landmark' },
  { id: 3, name: 'سويسرا', icon: 'fas fa-mountain' },
  { id: 4, name: 'دبي', icon: 'fas fa-mosque' },
  { id: 5, name: 'اليابان', icon: 'fas fa-torii-gate' },
  { id: 6, name: 'نيويورك', icon: 'fas fa-city' },
];

export const testimonials = [
  {
    id: 1,
    name: 'أحمد محمد',
    image: 'https://images.unsplash.com/photo-1560250097-0b93528c311a?ixlib=rb-4.0.3&auto=format&fit=crop&w=100&h=100',
    rating: 5,
    comment: '"تجربة رائعة مع شركة التَرَف! استفدت من خدمة استخراج التأشيرة بسرعة وسهولة، وكان التعامل مع قسم التذاكر احترافي جداً. الفندق كان فاخراً والخدمة متميزة والتنظيم دقيق. سأعتمد عليهم حتماً في سفرياتي القادمة."'
  },
  {
    id: 2,
    name: 'سارة عبدالله',
    image: 'https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?ixlib=rb-4.0.3&auto=format&fit=crop&w=100&h=100',
    rating: 4.5,
    comment: '"استمتعت كثيراً برحلتي إلى سويسرا التي نظمتها شركة التَرَف. خدمة الاستقبال والتوديع (الترانسفير) كانت ممتازة، والتأمين الصحي الذي قدموه كان شاملاً وأشعرني بالأمان طوال الرحلة. أنصح بشدة بالتعامل معهم."'
  },
  {
    id: 3,
    name: 'خالد العمري',
    image: 'https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?ixlib=rb-4.0.3&auto=format&fit=crop&w=100&h=100',
    rating: 5,
    comment: '"كرجل أعمال، أقدر كثيراً الاحترافية العالية التي يتمتع بها فريق التَرَف. قسم تنظيم المؤتمرات لديهم قدّم خدمة استثنائية في تنظيم مؤتمرنا الأخير، وخدمات استخراج التأشيرات وإجازات السوق الدولية كانت سريعة وفعالة. خدمة ممتازة تستحق كل تقدير."'
  },
  {
    id: 4,
    name: 'نور الحسيني',
    image: 'https://images.unsplash.com/photo-1580489944761-15a19d654956?ixlib=rb-4.0.3&auto=format&fit=crop&w=100&h=100',
    rating: 5,
    comment: '"سافرت مع عائلتي إلى دبي عبر شركة التَرَف، وكانت التجربة مميزة من البداية للنهاية. الحجوزات والفنادق كانت فاخرة، وخدمة الاستقبال كانت في الموعد تماماً. ما أعجبني هو الاهتمام بأدق التفاصيل والرد السريع على جميع استفساراتي. بالتأكيد سأختارهم مجدداً!"'
  },
  {
    id: 5,
    name: 'محمد القاسم',
    image: 'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?ixlib=rb-4.0.3&auto=format&fit=crop&w=100&h=100',
    rating: 4.5,
    comment: '"استخرجت إجازة السوق الدولية من شركة التَرَف، وكانت العملية سلسة وسريعة بشكل مذهل. الموظفون متعاونون جداً وقدموا لي جميع المعلومات بوضوح. كما استفدت من خدمة تأمينهم الصحي خلال سفري، وكانت التغطية شاملة ومريحة. خدمة ممتازة تستحق التجربة."'
  },
  {
    id: 6,
    name: 'ليلى العبدالله',
    image: 'https://images.unsplash.com/photo-1544005313-94ddf0286df2?ixlib=rb-4.0.3&auto=format&fit=crop&w=100&h=100',
    rating: 5,
    comment: '"كنت قلقة بشأن تنظيم رحلة زفافي إلى المالديف، لكن التَرَف تجاوزت جميع توقعاتي! كان لديهم باقة خاصة للعرسان مع خدمات حصرية ومفاجآت رائعة. الصور لا تعبر عن جمال المنتجع الذي اختاروه لنا، والتنسيق كان مثالياً. الشكر لفريقهم المحترف على جعل هذه المناسبة الخاصة لا تُنسى!"'
  },
  {
    id: 7,
    name: 'فاطمة الهاشمي',
    image: 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?ixlib=rb-4.0.3&auto=format&fit=crop&w=100&h=100',
    rating: 5,
    comment: '"رحلتي إلى اليابان مع شركة التَرَف كانت تجربة استثنائية! فريق العمل اختار الوجهات الأكثر تميزًا ونظم مسار الرحلة بشكل مدروس. الفنادق كانت راقية جدًا والمرشدين كانوا على درجة عالية من المعرفة والخبرة. أشكرهم على هذه التجربة الفريدة التي جمعت بين الراحة والمتعة والثقافة."'
  },
  {
    id: 8,
    name: 'عمر الحسن',
    image: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?ixlib=rb-4.0.3&auto=format&fit=crop&w=100&h=100',
    rating: 4.5,
    comment: '"حجزت تذاكر طيران وفندق لرحلة عائلية عبر شركة التَرَف، وكان الاختيار موفقًا جدًا! حصلت على أسعار تنافسية مع خدمات فاخرة، وكان التنسيق بين مواعيد الطيران والإقامة مثاليًا. أقدر لهم المرونة في تعديل بعض تفاصيل الحجز قبل السفر، وسأختارهم بلا تردد في رحلاتي القادمة."'
  },
  {
    id: 9,
    name: 'هدى النعيمي',
    image: 'https://images.unsplash.com/photo-1619895862022-09114b41f16f?ixlib=rb-4.0.3&auto=format&fit=crop&w=100&h=100',
    rating: 5,
    comment: '"بفضل شركة التَرَف، استطعت زيارة باريس بأسلوب راقٍ لم أتوقعه! كل التفاصيل كانت معدة بعناية فائقة، من خدمة الاستقبال الفاخرة في المطار إلى الإقامة في فندق يطل على برج إيفل. الجولات السياحية كانت خاصة وبعيدة عن الازدحام. شكرًا لجعل حلمي في زيارة باريس يتحقق بهذا المستوى من الفخامة!"'
  }
];
